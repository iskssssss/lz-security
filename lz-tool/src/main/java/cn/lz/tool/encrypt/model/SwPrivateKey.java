...
public class SwPrivateKey {
	...
	public String encrypt(String message) {
		return encryptString(message.getBytes(StandardCharsets.UTF_8));
	}
	 ...
	 * @return 密文
	 */
	public String encryptString(byte[] bytes) {
		try {
			return RSAEncryptUtil.encryptString(bytes, getPrivateKey());
		} catch (Exception e) {
	...
	public byte[] encrypt(byte[] bytes) {
		try {
			return RSAEncryptUtil.encryptByte(bytes, getPrivateKey());
		} catch (Exception e) {
			throw new RuntimeException("加密失败，请检查私钥或待加密信息。", e);
		}
	}
	...
	public Object decrypt(String message) {
		try {
			return RSAEncryptUtil.decrypt(message.getBytes(StandardCharsets.UTF_8), getPrivateKey());
		} catch (Exception e) {
	...
	public byte[] decrypt(byte[] bytes) {
		try {
			return RSAEncryptUtil.decryptByte(bytes, getPrivateKey());
		} catch (Exception e) {
			throw new RuntimeException("解密失败，请检查私钥或待解密信息。", e);
		}
	}
